...
train_loader, val_loader, test_loader, vocab = get_loader_and_vocab(dt, feature_type_one="KeypointDetection", feature_type_two="ObjectDetection")
...
def train_model(net, vocab, epochs, comment, learning_rate=0.01):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        json_file = f"results/{dt.name}_DATASET_object_dedection_and_keypoint_{val_annotation_name}_result.json"
        ...
        for metric, score in coco_val_eval.eval.items():
            ...
            if metric == "CIDEr":
                if BEST_VAL_CIDER_SCORE < score:
                    ...
                    with open(f"results/{dt.name}_DATASET_object_dedection_and_keypoint_{dt.name}_best_val_result.json", 'w') as file:
                        json.dump(val_data, file)
                    ...
                    json_file = f"results/{dt.name}_DATASET_object_dedection_and_keypoint_{dt.name}_best_test_result.json"
                    with open(json_file, "w") as file:
...
LEARNING_RATE = 0.01
EPOCH = 200
COMMENT = f"_ObjectDetection_and_KeypointDetection_{EPOCH}_EPOCH_{dt.name}_DATASET_{SEED}_SEED_NUMBER_{LEARNING_RATE}_LEARNING_RATE"
