...
class FeatureExtraction():
    def __init__(self, input_folder:Path, output_folder:Path, model:torch.nn.Module, IMAGE_SIZE:int, params:dict) -> None:
        ...
        self.output_folder = output_folder / "SemanticSegmantation"
        self.model = model
    ...
    def run(self) -> None:
        ...
        for local_batch, local_ids in tqdm(self.generator):
            ...
            with torch.no_grad():
                ...
                predictions = self.avgpool(predictions['out']).squeeze()
                for i, feature in enumerate(predictions):
                    file_name = Path(local_ids[i]).stem
                    #torch.save(feature.cpu(), self.output_folder / f'{file_name}_SemanticSegmentation.pt')
